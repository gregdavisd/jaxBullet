...
public class AllConstraintDemo extends DiscreteDemoContainer {
 ...
 public void initPhysics() {
  ...
  btCollisionShape shape = new btBoxShape(new btVector3(CUBE_HALF_EXTENTS,
   CUBE_HALF_EXTENTS,
   CUBE_HALF_EXTENTS));
...
///gear constraint demo
   ...
   btRigidBody bodyB = null;
   {
    ...
    btRigidBodyConstructionInfo ci = new btRigidBodyConstructionInfo(mass, null,
     cyl0, localInertia);
   ...
   btGearConstraint gear = new btGearConstraint(bodyA, bodyB, axisA, axisB,
    RATIO);
  ...
  if (ENABLE_ALL_DEMOS || POINT_2_POINT) {
   //point to point constraint with a breaking threshold
   {
    ...
    final btVector3 pivotInA = new btVector3(CUBE_HALF_EXTENTS,
     CUBE_HALF_EXTENTS, 0);
...
//	btRigidBody* pRbB1 = createRigidBody(0.f, trans, shape);
    ...
    btSliderConstraint spSlider1 = new btSliderConstraint(pRbA1, pRbB1, frameInA,
     frameInB, true);
...
//	spSlider1.setUpperLinLimit(-10.0F);
  ...
  if (ENABLE_ALL_DEMOS || SLIDER_D6) {
   //create a slider, using the generic D6 constraint
   {
    ...
    final btMatrix3x3 sliderOrientation = new btMatrix3x3(new btQuaternion(
     sliderAxis, angle));
    trans.setIdentity();
...
//		bool useLinearReferenceFrameA = false;//use fixed frame B for linear llimits
    ...
    spSlider6Dof = new btGeneric6DofConstraint(fixedBody1, d6body0, frameInA,
     frameInB,
...
//		btRigidBody* pBodyB = createRigidBody(0.f, tr, shape);
    ...
    btGeneric6DofConstraint pGen6DOF = new btGeneric6DofConstraint(pBodyA,
     pBodyB, frameInA,
     frameInB, true);
...
//		pGen6DOF.setAngularUpperLimit(btVector3(0., 0., SIMD_HALF_PI));
    pGen6DOF.setAngularLowerLimit(new btVector3(-SIMD_HALF_PI * 0.5f, -0.75f,
     -SIMD_HALF_PI * 0.8f));
    pGen6DOF.setAngularUpperLimit(new btVector3(SIMD_HALF_PI * 0.5f, 0.75f,
     SIMD_HALF_PI * 0.8f));
...
//		pHinge.enableAngularMotor(true, -1.0, 0.165); // use for the old solver
  ...
  if (ENABLE_ALL_DEMOS || UNIVERSAL) {
   {
    ...
    btUniversalConstraint pUniv = new btUniversalConstraint(pBodyA, pBodyB,
     anchor, parentAxis,
     childAxis);
  ...
  if (ENABLE_ALL_DEMOS || HINGE_DYNAMIC) {
   ...
   spHingeDynAB = new btHingeConstraint(pBodyA, pBodyB, pivotA, pivotB, axisA,
    axisB);
  ...
  if (ENABLE_ALL_DEMOS || MOTOR_6DOF) { // 6DOF connected to the world, with motor
   ...
   frameB.setIdentity();
   btGeneric6DofConstraint pGen6Dof = new btGeneric6DofConstraint(pBody, frameB,
    false);
 ...
 public void resetCamera() {
  camera().set(new btQuaternion().set(2.085825E-4f, -0.9988975f, -0.046732347f,
   0.0044579273f),
