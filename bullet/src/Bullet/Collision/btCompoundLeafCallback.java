 ...
 */
public class btCompoundLeafCallback extends btDbvt.ICollide implements
 Serializable {
 ...
 public btCompoundLeafCallback(btCollisionObjectWrapper compoundObjWrap,
  btCollisionObjectWrapper otherObjWrap, btDispatcher dispatcher,
  btDispatcherInfo dispatchInfo,
  btManifoldResult resultOut,
  List<btCollisionAlgorithm> childCollisionAlgorithms,
 ...
 public void processChildShape(btCollisionShape childShape, int index) {
  assert (index >= 0);
  btCompoundShape compoundShape = (btCompoundShape) (m_compoundColObjWrap
   .getCollisionShape());
  ...
  final btTransform newChildWorldTrans = new btTransform(orgTrans).mul(
   childTrans);
  //perform an AABB check first
  final btVector3 aabbMin0 = new btVector3();
  final btVector3 aabbMax0 = new btVector3();
  childShape.getAabb(newChildWorldTrans, aabbMin0, aabbMax0);
  final btVector3 extendAabb = new btVector3(
  ...
  m_otherObjWrap.getCollisionShape().getAabb(m_otherObjWrap.getWorldTransform(),
   aabbMin1, aabbMax1);
  if (gCompoundChildShapePairCallback != null) {
   if (!gCompoundChildShapePairCallback.callback(m_otherObjWrap
    .getCollisionShape(), childShape)) {
    return;
   }
  }
  if (TestAabbAgainstAabb2(aabbMin0, aabbMax0, aabbMin1, aabbMax1)) {
   btCollisionObjectWrapper compoundWrap = new btCollisionObjectWrapper(
    this.m_compoundColObjWrap,
    childShape, m_compoundColObjWrap.getCollisionObject(), newChildWorldTrans,
    -1, index);
   ...
   if (m_resultOut.m_closestPointDistanceThreshold > 0) {
    ...
    if (m_childCollisionAlgorithms.get(index) == null) {
     m_childCollisionAlgorithms.set(index, m_dispatcher.findAlgorithm(
      compoundWrap, m_otherObjWrap,
