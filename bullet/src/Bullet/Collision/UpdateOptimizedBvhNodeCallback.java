...
class UpdateOptimizedBvhNodeCallback implements btTriangleCallback, Serializable {
 ...
 UpdateOptimizedBvhNodeCallback(btOptimizedBvh owner) {
  this.owner = owner;
  curNode = null;
 }

 void setCurNode(btQuantizedBvhNode curNode) {
 ...
 public boolean processTriangle(btVector3[] triangle, int partId, int triangleIndex) {
  ...
  if (curNode != null) {
   owner.quantize(curNode.m_quantizedAabbMin, aabbMin, 0);
   owner.quantize(curNode.m_quantizedAabbMax, aabbMax, 1);
  }
  // process only one triangle
  return false;
 }
}
