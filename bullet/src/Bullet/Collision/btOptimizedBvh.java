...
public class btOptimizedBvh extends btQuantizedBvh implements Serializable {
 ...
 public void build(btStridingMeshInterface triangles,
  ...
  if (m_useQuantization) {
   ...
   QuantizedNodeTriangleCallback callback = new QuantizedNodeTriangleCallback(
    m_quantizedLeafNodes,
    this);
   triangles.InternalProcessAllTriangles(callback, new btVector3(),
    new btVector3());
   ...
   BuildOptimizedBvhNodeCallback callback = new BuildOptimizedBvhNodeCallback(
    m_leafNodes);
 ...
 public void refit(btStridingMeshInterface meshInterface,
  ...
  if (m_useQuantization) {
   ...
   for (i = 0; i < m_SubtreeHeaders.size(); i++) {
    ...
    subtree.setAabbFromQuantizeNode(m_quantizedContiguousNodes.get(
 ...
 public void refitPartial(btStridingMeshInterface meshInterface,
  final btVector3 aabbMin,
  final btVector3 aabbMax) {
 ...
 public void updateBvhNodes(btStridingMeshInterface meshInterface, int firstNode,
  int endNode,
  int index) {
  assert (m_useQuantization);
...
//  final btVector3 meshScaling = meshInterface.getScaling();
  UpdateOptimizedBvhNodeCallback callback = new UpdateOptimizedBvhNodeCallback(
   this);
...
//						meshInterface.unLockReadOnlyVertexBase(curNodeSubPart);
    ...
    meshInterface.InternalProcessSubPart(callback, mesh_lock, aabbMin, aabbMax,
     nodeSubPart,
     nodeTriangleIndex);
   } else {
    //combine aabb from both children
    btQuantizedBvhNode leftChildNode = m_quantizedContiguousNodes.get(i + 1);
    btQuantizedBvhNode rightChildNode = leftChildNode.isLeafNode() ? m_quantizedContiguousNodes
     .get(
      i + 2)
