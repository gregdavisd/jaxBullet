 ...
 */
public class btPerturbedContactResult extends btManifoldResult implements
 Serializable {
 ...
 public btPerturbedContactResult(btManifoldResult originalResult,
  final btTransform transformA,
  final btTransform transformB, final btTransform unPerturbedTransform,
  boolean perturbA,
 ...
 public void addContactPoint(final btVector3 normalOnBInWorld,
  ...
  float newDepth;
  if (m_perturbA) {
   final btVector3 endPtOrg = new btVector3(new btVector3(pointInWorld).add(
    new btVector3(
     normalOnBInWorld).scale(orgDepth)));
   endPt.set((new btTransform(m_unPerturbedTransform).mul(new btTransform(
    m_transformA).invert()))
   ...
   startPt.set((new btTransform(m_unPerturbedTransform).mul(new btTransform(
    m_transformB).invert()))
