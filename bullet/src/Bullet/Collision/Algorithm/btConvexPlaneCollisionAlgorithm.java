...
/**
 * btSphereBoxCollisionAlgorithm provides sphere-box collision detection. Other features are
 * frame-coherency (persistent data) and collision response.
 *
 * @author Gregery Barton
 */
public class btConvexPlaneCollisionAlgorithm extends btCollisionAlgorithm  implements Serializable {
 ...
 btConvexPlaneCollisionAlgorithm(btPersistentManifold mf, btCollisionAlgorithmConstructionInfo ci,
  ...
  if (m_manifoldPtr == null && m_dispatcher.needsCollision(convexObjWrap.getCollisionObject(),
   planeObjWrap.getCollisionObject())) {
   m_manifoldPtr = m_dispatcher.getNewManifold(convexObjWrap.getCollisionObject(), planeObjWrap
    .getCollisionObject());
   m_ownManifold = true;
  }
 }
 ...
 public void processCollision(btCollisionObjectWrapper body0Wrap, btCollisionObjectWrapper body1Wrap,
  ...
  final btTransform planeInConvex = convexObjWrap.getWorldTransform().invert().mul(planeObjWrap
   .getWorldTransform());
  final btTransform convexInPlaneTrans = planeObjWrap.getWorldTransform().invert().mul(convexObjWrap
   .getWorldTransform());
  ...
  final btVector3 vtxInPlaneProjected = new btVector3().scaleAdd(-distance, planeNormal, vtxInPlane);
  final btVector3 vtxInPlaneWorld = planeObjWrap.getWorldTransform().transform(new btVector3(
   vtxInPlaneProjected));
  hasCollision = distance < m_manifoldPtr.getContactBreakingThreshold();
  resultOut.setPersistentManifold(m_manifoldPtr);
  if (hasCollision) {
   /// report a contact. internally this will be kept persistent, and contact reduction is done
   final btVector3 normalOnSurfaceB = planeObjWrap.getWorldTransform().transform3x3(new btVector3(
    planeNormal));
  ...
  if (convexShape.isPolyhedral() && resultOut.getPersistentManifold().getNumContacts() <
   ...
   for (int i = 0; i < m_numPerturbationIterations; i++) {
    ...
    final btQuaternion rotq = new btQuaternion().set(planeNormal, iterationAngle);
    collideSingleContact(new btQuaternion(rotq).conjugate().mul(perturbeRot).mul(rotq), body0Wrap,
 ...
 void collideSingleContact(final btQuaternion perturbeRot, btCollisionObjectWrapper body0Wrap,
  ...
  convexInPlaneTrans = planeObjWrap.getWorldTransform().invert().mul(convexWorldTransform);
  //now perturbe the convex-world transform
  convexWorldTransform.setBasis(convexWorldTransform.getBasis().mul(new btMatrix3x3().set(
   perturbeRot)));
  final btTransform planeInConvex;
  planeInConvex = new btTransform(convexWorldTransform).invert().mul(planeObjWrap
   .getWorldTransform());
  final btVector3 vtx = convexShape.localGetSupportingVertex(planeInConvex.transform3x3(
   new btVector3(
    planeNormal).negate()));
...
public   btCollisionAlgorithm CreateCollisionAlgorithm(btCollisionAlgorithmConstructionInfo ci,
   ...
   if (!m_swapped) {
    return new btConvexPlaneCollisionAlgorithm(null, ci, body0Wrap, body1Wrap, false,
     m_numPerturbationIterations, m_minimumPointsPerturbationThreshold);
   } else {
    return new btConvexPlaneCollisionAlgorithm(null, ci, body0Wrap, body1Wrap, true,
     m_numPerturbationIterations, m_minimumPointsPerturbationThreshold);
   }
  }
 };
