...
public class SphereTriangleDetector extends btDiscreteCollisionDetectorInterface
 ...
 static float SegmentSqrDistance(final btVector3 from, final btVector3 to,
  final btVector3 p,
...
//move sphere into triangle space
  ...
  if (collide(sphereInTr.getOrigin(), point, normal, depth, timeOfImpact,
   ...
   if (swapResults) {
    ...
    final btVector3 pointOnA = new btVector3().scaleAdd(depth[0], normalOnB,
     transformB.transform(
      new btVector3(point)));
    output.addContactPoint(normalOnA, pointOnA, depth[0]);
   } else {
    output.addContactPoint(transformB.transform3x3(new btVector3(normal)),
     transformB.transform(
 ...
 boolean collide(final btVector3 sphereCenter, final btVector3 point,
  final btVector3 resultNormal,
  ...
  final btVector3 normal = new btVector3(vertex1).sub(vertex0).cross(
   new btVector3(vertex2).sub(
    vertex0));
  ...
  if (hasContact) {
   final btVector3 contactToCentre = new btVector3(sphereCenter).sub(
    contactPoint);
 ...
 boolean facecontains(final btVector3 p, btVector3[] vertices,
 ...
 boolean pointInTriangle(btVector3 vertices[], final btVector3 normal,
  final btVector3 p) {
