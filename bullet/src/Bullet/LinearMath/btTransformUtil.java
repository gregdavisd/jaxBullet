...
public class btTransformUtil implements Serializable {
  ...
  * @param angVel
  */
 public static void calculateVelocity(final btTransform transform0,
  final btTransform transform1,
  float timeStep, final btVector3 linVel, final btVector3 angVel) {
  linVel.set(transform1.getOrigin()).sub(transform0.getOrigin()).scale(1.0f
   / timeStep);
 ...
 public static float calculateDiffAxisAngle(final btTransform transform0,
  ...
  final btMatrix3x3 dmat = transform1.getBasis().mul(transform0.getBasis()
   .invert());
  ...
  */
 public static void integrateTransform(final btTransform curTrans,
...
//  assert (timeStep >= 0);
  final btVector3 predictedOrigin = new btVector3().scaleAdd(timeStep, linvel,
   curTrans.getOrigin());
  //Exponential map
  ...
  final btQuaternion dorn
   = new btQuaternion(axis.x(), axis.y(), axis.z(), btCos(fAngle * timeStep
    * 0.5f));
