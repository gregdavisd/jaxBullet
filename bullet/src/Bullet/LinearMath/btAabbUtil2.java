...
public class btAabbUtil2 implements Serializable {
  ...
  * @return
  */
 public static boolean btRayAabb2(final btVector3 rayFrom,
  final btVector3 rayInvDirection,
  int[] raySign,
  btVector3[] bounds,
  float[] tmin,
  float lambda_min,
  ...
  tmin[0] = (bounds[raySign[0]].getX() - rayFrom.getX()) * rayInvDirection
   .getX();
  tmax = (bounds[1 - raySign[0]].getX() - rayFrom.getX()) * rayInvDirection
   .getX();
  tymin = (bounds[raySign[1]].getY() - rayFrom.getY()) * rayInvDirection.getY();
  tymax = (bounds[1 - raySign[1]].getY() - rayFrom.getY()) * rayInvDirection
   .getY();
  ...
  tzmax = (bounds[1 - raySign[2]].getZ() - rayFrom.getZ()) * rayInvDirection
   .getZ();
 ...
 public static boolean btRayAabb(final btVector3 rayFrom, final btVector3 rayTo,
  ...
  final btVector3 aabbHalfExtent = (new btVector3(aabbMax).sub(aabbMin)).scale(
   0.5f);
   ...
   for (int j = 0; j < 2; j++) {
    for (i = 0; i != 3; ++i) {
     if ((sourceOutcode & bit) != 0) {
      float lambda = (-source.getElement(i) - aabbHalfExtent.getElement(i)
       * normSign) / r
        .getElement(i);
 ...
 public static int btOutcode(final btVector3 p, final btVector3 halfExtent) {
  return (p.getX() < -halfExtent.getX() ? 0x01 : 0x0) | (p.getX() > halfExtent
   .getX() ? 0x08 : 0x0) | (p.getY() < -halfExtent.getY() ? 0x02 : 0x0) | (p
   .getY() > halfExtent.getY() ? 0x10 : 0x0)
  ...
  * @param aabbMaxOut
  */
 public static void btTransformAabb(final btVector3 halfExtents, float margin,
  final btTransform t,
  final btVector3 aabbMinOut, final btVector3 aabbMaxOut) {
  final btVector3 halfExtentsWithMargin = new btVector3(halfExtents).add(
   new btVector3(margin,
    margin,
    margin));
...
//   .getRow(2));
  ...
  * @param aabbMaxOut
  */
 public static void btTransformAabb(final btVector3 localAabbMin,
  final btVector3 localAabbMax,
  float margin, final btTransform trans, final btVector3 aabbMinOut,
  final btVector3 aabbMaxOut) {
  ...
  final btVector3 localHalfExtents = new btVector3(localAabbMax).sub(
   localAabbMin).scale(0.5f);
  localHalfExtents.add(new btVector3(margin, margin, margin));
  final btVector3 localCenter = new btVector3(localAabbMax).add(localAabbMin)
   .scale(0.5f);
  ...
  * @return
  */
 public static boolean TestTriangleAgainstAabb2(btVector3[] vertices,
...
/// conservative test for overlap between two aabbs
  ...
  * @return
  */
 public static boolean TestAabbAgainstAabb2(final btVector3 aabbMin1,
  final btVector3 aabbMax1,
  final btVector3 aabbMin2, final btVector3 aabbMax2) {
  boolean overlap = true;
  overlap = (aabbMin1.getX() > aabbMax2.getX() || aabbMax1.getX() < aabbMin2
   .getX()) ? false
    : overlap;
  overlap = (aabbMin1.getZ() > aabbMax2.getZ() || aabbMax1.getZ() < aabbMin2
   .getZ()) ? false
    : overlap;
  overlap = (aabbMin1.getY() > aabbMax2.getY() || aabbMax1.getY() < aabbMin2
   .getY()) ? false
    : overlap;
  return overlap;
 }
  ...
  * @return
  */
 public static int testQuantizedAabbAgainstQuantizedAabb(short[] aabbMin1,
  short[] aabbMax1,
  short[] aabbMin2, short[] aabbMax2) {
  return (btSelect(((aabbMin1[0] <= aabbMax2[0]) & (aabbMax1[0] >= aabbMin2[0])
   & (aabbMin1[2] <= aabbMax2[2]) & (aabbMax1[2] >= aabbMin2[2]) & (aabbMin1[1]
   <= aabbMax2[1]) & (aabbMax1[1] >= aabbMin2[1])) ? 1 : 0,
   1, 0));
 }
