...
public  int m_orgIndex;
 ...
 public static void GrahamScanConvexHull2D(ArrayList<GrahamVector3> originalPoints, ArrayList<GrahamVector3>  hull, final btVector3 normalAxis)
{
   ...
   Collections.swap(originalPoints, 0, i);
	...
	for (; i != originalPoints.size(); i++) 
	{
		...
		while (!isConvex&& hull.size()>1) {
			...
			isConvex = new btVector3(a).sub(b).cross(new btVector3(a).sub(originalPoints.get(i))).dot(normalAxis)> 0;
			if (!isConvex)
