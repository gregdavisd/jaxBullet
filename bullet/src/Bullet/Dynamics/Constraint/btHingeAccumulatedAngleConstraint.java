...
/**
 * The getAccumulatedHingeAngle returns the accumulated hinge angle, taking rotation across the
 * -PI/PI boundary into account
 *
...
public class btHingeAccumulatedAngleConstraint extends btHingeConstraint {
 ...
 static float btShortestAngularDistance(float accAngle, float curAngle) {
  float result = btNormalizeAngle(btNormalizeAnglePositive(btNormalizeAnglePositive(curAngle) -
   btNormalizeAnglePositive(accAngle)));
  return result;
 }

 static float btNormalizeAnglePositive(float angle) {
  return btFmod(btFmod(angle, (2.0f * SIMD_PI)) + (2.0f * SIMD_PI), (2.0f * SIMD_PI));
 }
};
