 ...
 */
public abstract class btTypedConstraint extends btTypedObject implements
 Serializable {
 ...
 public static float btAdjustAngleToLimits(float angleInRadians,
  ...
  if (angleLowerLimitInRadians >= angleUpperLimitInRadians) {
   ...
   float diffLo = btFabs(btNormalizeAngle(angleLowerLimitInRadians
    - angleInRadians));
   float diffHi = btFabs(btNormalizeAngle(angleUpperLimitInRadians
    - angleInRadians));
   return (diffLo < diffHi) ? angleInRadians : (angleInRadians + SIMD_2_PI);
  } else if (angleInRadians > angleUpperLimitInRadians) {
   float diffHi = btFabs(btNormalizeAngle(angleInRadians
    - angleUpperLimitInRadians));
   float diffLo = btFabs(btNormalizeAngle(angleInRadians
 ...
 public float getMotorFactor(float pos, float lowLim, float uppLim, float vel,
  float timeFact) {
  if (lowLim > uppLim) {
   return (1.0f);
 ...
 ///internal method used by the constraint solver, don't use them directly
 public void setupSolverConstraint(ArrayList<btSolverConstraint> ca,
  int solverBodyA,
