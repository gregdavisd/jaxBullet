 ...
 */
public class InplaceSolverIslandCallback extends btSimulationIslandManager.IslandCallback
 implements
 Serializable {
 ...
 void setup(btContactSolverInfo solverInfo,
  ArrayList<btTypedConstraint> sortedConstraints,
 ...
 public void processIsland(List<btCollisionObject> bodies, int numBodies,
  ...
  if (islandId < 0) {
   ...
   if (m_solverInfo.m_minimumSolverBatchSize <= 1) {
    m_solver.solveGroup(bodies, numBodies, manifolds, numManifolds,
     (ArrayList) m_sortedConstraints
      .subList(
       startConstraint, startConstraint + numCurConstraints), numCurConstraints,
     m_solverInfo,
...
//		ArrayList<btTypedConstraint> constraints = !m_constraints.isEmpty()?m_constraints:null;
  m_solver
   .solveGroup(m_bodies, m_bodies.size(), m_manifolds, m_manifolds.size(),
    m_constraints,
